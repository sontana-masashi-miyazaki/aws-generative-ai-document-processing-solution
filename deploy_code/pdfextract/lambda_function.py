...
def lambda_handler(event, context):
    ...
    assets_manifest_key = event.get("structured_assets_manifest_key") or posixpath.join(structured_prefix, "assets_manifest.json")
    ...
    succeeded = len(objects) > 0 and status in {"SUCCEEDED", "PARTIAL_SUCCESS"}
    ...
    manifest = {
        ...
        "output": {
            ...
            "assets": {"images_prefix": event.get("assets_images_prefix")},
        },
        "textract": {
            "job_id": job_id,
            "status": status,
    ...
    # Required empty assets manifest for downstream stages.
    S3.put_object(
        Bucket=processing_bucket,
